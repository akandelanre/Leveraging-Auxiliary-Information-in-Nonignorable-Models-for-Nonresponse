...
#### Vote excluded in item nonresponse models for gender and age ##########
###########################################################################
###########################################################################
...
PostPred_ContTable <- function(PostParameters,State){
  ...
  pi_W <- beta_W1 + beta_W2[NewData$state] + beta_W3[NewData$age]
  pi_W <- 1/(exp(-pi_W)+1)
  ...
  beta_R_vote5 <- PostParameters[c("beta_R_vote5")]
  pi_R_vote <- beta_R_vote1 + beta_R_vote2[NewData$state] + beta_R_vote3[NewData$age] + 
    beta_R_vote4*NewData$gender + beta_R_vote5*NewData$vote
...
ProbVote_Posterior <- function(Model_mat_sub){
  ...
  for(j in 1:nrow(Model_mat_sub)){
    ...
    ExpGrid_st_ag <- expand.grid(state=(1:4),age=c(1:4))
    ...
    Pr_u_giv_s_a_g_v <- apply(ExpGrid_st_ag,1,function(x) beta_W1 + beta_W2[x["state"]] + beta_W3[x["age"]])
    ...
    beta_R_vote5 <- PostParameters_j["beta_R_vote5"]
    Pr_r_vote_giv_s_a_g_v <- apply(ExpGrid_st_ag_gd_vt,1,function(x) beta_R_vote1 + beta_R_vote2[x["state"]] + 
                                     beta_R_vote3[x["age"]] + beta_R_vote4*x["gender"] + beta_R_vote5*x["vote"])
      ...
      cond_table_with_r_vote$Prob[k] <- cond_table_with_r_vote$Prob[k]*
        r_vote_by_st_ag_gd_vt$Prob[is.element(r_vote_by_st_ag_gd_vt$state,cond_table_with_r_vote$state[k]) &
                                is.element(r_vote_by_st_ag_gd_vt$age,cond_table_with_r_vote$age[k]) &
                                  ...
                                  is.element(r_vote_by_st_ag_gd_vt$vote,cond_table_with_r_vote$vote[k])]
    }
...
#compute nonresponse rates by state
by(Data,list(State=Data$state),function(x) sum(x$unit==1)/length(x$unit))
by(R[Data$unit==0,],list(State=Data$state[Data$unit==0]),function(x) sum(x$age==1)/length(x$age))
by(R[Data$unit==0,],list(State=Data$state[Data$unit==0]),function(x) sum(x$gender==1)/length(x$gender))
by(R[Data$unit==0,],list(State=Data$state[Data$unit==0]),function(x) sum(x$vote==1)/length(x$vote))
...
by(Aug_Data$vote[-c(1:n)],list(State=Aug_Data$state[-c(1:n)]),function(x) table(x)/sum(table(x)))
by(Aug_Data$vote[c(1:n)],list(State=Aug_Data$state[c(1:n)]),function(x) table(x))

by(Aug_Data$age[-c(1:n)],list(State=Aug_Data$state[-c(1:n)]),function(x) table(x))
by(Aug_Data$age[c(1:n)],list(State=Aug_Data$state[c(1:n)]),function(x) table(x))
...
W[i] ~ dbern(pi_W[i])
logit(pi_W[i]) = beta_W1 + beta_W2[state[i]] + beta_W3[age[i]]
...
logit(pi_R_vote[i]) = beta_R_vote1 + beta_R_vote2[state[i]] + beta_R_vote3[age[i]] + 
  beta_R_vote4*gender[i] + beta_R_vote5*vote[i]
}
...
#png("Figures/PosteriorPredictiveCheck_NewData.png",pointsize=12,width = 8, height = 6,bg="white",units="in",res=150)
postscript("Figures/PosteriorPredictiveCheck_NewData_MD-R.eps",
...
#png("DeviationInVoterCompositionEstimates_PostEst_NewData.png",pointsize=11,width = 13, height = 7,bg="white",units="in",res=150)
#plot_ly(type = 'box') %>% 
...
#png("Figures/DeviationInVoterCompositionEstimates_PostEst_NewData.png",pointsize=8,width = 6, height = 4,bg="white",units="in",res=150)
postscript("Figures/DeviationInVoterCompositionEstimates_PostEst_NewData_MD-R.eps",
...
#png("VoterTurnoutForUnitNonrespondents_Boxplot_PostEst_NewData.png",pointsize=11,width = 13, height = 7,bg="white",units="in",res=150)
#plot_ly(type = 'box') %>% 
...
#png("VoterTurnoutForUnitNonrespondents_Violin_PostEst_NewData.png",pointsize=11,width = 13, height = 7,bg="white",units="in",res=150)
#PostPredTurnout %>%
...
#  layout(xaxis = list(title = "State"),yaxis = list(title = "Turnout",zeroline = F,range = c(0.48, 0.66)))
#dev.off()


postscript("Figures/VoterTurnoutForUnitNonrespondents_Violin_PostEst_NewData_MD-R.eps",
           width = 6.5, height = 3.5, horizontal = FALSE, onefile = FALSE)
...
postscript("Figures/VoterTurnoutForItemNonrespondentsVote_Violin_PostEst_NewData_MD-R_rescaled.eps",
           width = 6.5, height = 3.5, horizontal = FALSE, onefile = FALSE)
ggplot(PostPredTurnout_Vote, aes(y = Turnout, x = State)) +
  ...
  scale_color_manual(values=c("tomato3","tomato2","tomato4","tomato1")) +
  #scale_color_grey() +
  scale_y_continuous(name = "Turnout",breaks = seq(0, 0.3, 0.1),limits=c(0,0.3)) +
  scale_x_discrete(name = "State") +
  theme_classic() #+
...
PostProb_Vote <- function(PostParameters){
  PostParameters_Mean <- PostParameters[,"Post mean"]
  # PostParameters_Mean <- Model_mat_sub[500,]
  
  # Vote given state, age and gender
  ExpGrid_st_ag_gd <- expand.grid(state=(1:4),gender=c(0,1),age=c(1:4))
  beta_vote1 <- PostParameters_Mean["beta_vote1"]
  beta_vote2 <- unlist(lapply(unique(ExpGrid_st_ag_gd$state),function(x) 
    PostParameters_Mean[paste0("beta_vote2[",x,"]")]))
  beta_vote3 <- unlist(lapply(unique(ExpGrid_st_ag_gd$age),function(x) 
    PostParameters_Mean[paste0("beta_vote3[",x,"]")]))
  beta_vote4 <- PostParameters_Mean["beta_vote4"]
  beta_vote5 <- matrix(0,ncol=4,nrow=4)
  beta_vote5[1,] <- PostParameters_Mean[c("beta_vote5[1,1]","beta_vote5[1,2]","beta_vote5[1,3]","beta_vote5[1,4]")]
  beta_vote5[2,] <- PostParameters_Mean[c("beta_vote5[2,1]","beta_vote5[2,2]","beta_vote5[2,3]","beta_vote5[2,4]")]
  beta_vote5[3,] <- PostParameters_Mean[c("beta_vote5[3,1]","beta_vote5[3,2]","beta_vote5[3,3]","beta_vote5[3,4]")]
  beta_vote5[4,] <- PostParameters_Mean[c("beta_vote5[4,1]","beta_vote5[4,2]","beta_vote5[4,3]","beta_vote5[4,4]")]
  Pr_v_giv_s_a_g <- apply(ExpGrid_st_ag_gd,1,function(x) 
    beta_vote1 + beta_vote2[x["state"]] + beta_vote3[x["age"]] + beta_vote4*x["gender"] + beta_vote5[x["state"],x["age"]])
  Pr_v_giv_s_a_g <- data.frame(Pr_vote = 1/(exp(-Pr_v_giv_s_a_g)+1))
  cond_table <- cbind(Pr_v_giv_s_a_g,ExpGrid_st_ag_gd)
  cond_table_vt_0 <- cond_table;
  cond_table_vt_0$Pr_vote = 1-cond_table_vt_0$Pr_vote
  vt_by_st_ag_gd <- cond_table
  vt_by_st_ag_gd$Pr_vote <- round(vt_by_st_ag_gd$Pr_vote,2)
  
  # Vote and age given state and gender
  ExpGrid_st_gd <- expand.grid(state=(1:4),gender=c(0,1))
  Pr_a_giv_s_g <- cumm_a_giv_s_g <- matrix(0,ncol=4,nrow=8)
  beta_age1 <- unlist(lapply(c(1:3),function(x) 
    PostParameters_Mean[paste0("beta_age1[",x,"]")]))
  beta_age2 <- unlist(lapply(unique(ExpGrid_st_gd$state),function(x) 
    PostParameters_Mean[paste0("beta_age2[",x,"]")]))
  beta_age3 <- PostParameters_Mean["beta_age3"]
  cumm_a_giv_s_g[,1] <- beta_age1[1] + beta_age2[ExpGrid_st_gd$state] + beta_age3*ExpGrid_st_gd$gender
  cumm_a_giv_s_g[,1] <- 1/(exp(-cumm_a_giv_s_g[,1])+1)
  Pr_a_giv_s_g[,1] <- cumm_a_giv_s_g[,1]
  for(j in 2:3){
  ...
  Pr_a_giv_s_g[,4] <- 1 - cumm_a_giv_s_g[,3]
  for(ag in 1:4){
    cond_table[cond_table$age==ag,"Pr_vote"] <- 
      cond_table[cond_table$age==ag,"Pr_vote"] * Pr_a_giv_s_g[,ag]
    cond_table_vt_0[cond_table_vt_0$age==ag,"Pr_vote"] <- 
      cond_table_vt_0[cond_table_vt_0$age==ag,"Pr_vote"] * Pr_a_giv_s_g[,ag]
  }
  vt_by_st_gd <- aggregate(cond_table$Pr_vote,
                           by=list(state=cond_table$state,gender=cond_table$gender), FUN=sum)
  colnames(vt_by_st_gd)[colnames(vt_by_st_gd)=="x"] <- "Pr_vote"
  vt_by_st_gd <- vt_by_st_gd[,c(3,1,2)]
  vt_by_st_gd$Pr_vote <- round(vt_by_st_gd$Pr_vote,2)
  
  
  # Vote, gender and age given state
  ExpGrid_st <- expand.grid(state=(1:4))
  beta_gender1 <- PostParameters_Mean["beta_gender1"]
  beta_gender2 <- unlist(lapply(unique(ExpGrid_st$state),function(x) 
    PostParameters_Mean[paste0("beta_gender2[",x,"]")]))
  Pr_g_giv_s <- apply(ExpGrid_st,1,function(x) beta_gender1 + beta_gender2[x["state"]])
  Pr_g_giv_s <- 1/(exp(-Pr_g_giv_s)+1)
  cond_table[cond_table$gender==1,"Pr_vote"] <- 
    cond_table$Pr_vote[cond_table$gender==1]*rep(Pr_g_giv_s,4)
  cond_table[cond_table$gender==0,"Pr_vote"] <- 
    cond_table$Pr_vote[cond_table$gender==0]*(1-rep(Pr_g_giv_s,4))
  cond_table_vt_0[cond_table_vt_0$gender==1,"Pr_vote"] <- 
    cond_table_vt_0$Pr_vote[cond_table_vt_0$gender==1]*rep(Pr_g_giv_s,4)
  cond_table_vt_0[cond_table_vt_0$gender==0,"Pr_vote"] <- 
    cond_table_vt_0$Pr_vote[cond_table_vt_0$gender==0]*(1-rep(Pr_g_giv_s,4))
  
  sum_cond_table <- cond_table
  sum_cond_table$Pr_vote <- sum_cond_table$Pr_vote + cond_table_vt_0$Pr_vote
  vt_by_st_ag <- aggregate(cond_table$Pr_vote,by=list(state=cond_table$state,
                                                      age=cond_table$age), FUN=sum)
  vt_by_st_ag$x <- vt_by_st_ag$x/
    (aggregate(sum_cond_table$Pr_vote,by=list(state=sum_cond_table$state,
                                              age=sum_cond_table$age),FUN=sum))$x
  colnames(vt_by_st_ag)[colnames(vt_by_st_ag)=="x"] <- "Pr_vote"
  vt_by_st_ag <- vt_by_st_ag[,c(3,1,2)]
  vt_by_st_ag$Pr_vote <- round(vt_by_st_ag$Pr_vote,2)
  
  
  vt_by_st <- aggregate(cond_table$Pr_vote,by=list(state=cond_table$state), FUN=sum)
  colnames(vt_by_st)[colnames(vt_by_st)=="x"] <- "Pr_vote"
  vt_by_st <- vt_by_st[,c(2,1)]
  vt_by_st$Pr_vote <- round(vt_by_st$Pr_vote,2)
  
  vt_by_st_ag_gd$state <- factor(vt_by_st_ag_gd$state,levels=c(1:4),labels=c("FL","GA","NC","SC"))
  vt_by_st_ag$state <- factor(vt_by_st_ag$state,levels=c(1:4),labels=c("FL","GA","NC","SC"))
  vt_by_st_gd$state <- factor(vt_by_st_gd$state,levels=c(1:4),labels=c("FL","GA","NC","SC"))
  vt_by_st$state <- factor(vt_by_st$state,levels=c(1:4),labels=c("FL","GA","NC","SC"))
  vt_by_st_ag_gd$age <- factor(vt_by_st_ag_gd$age,levels=c(1:4),labels=c("AGE1","AGE2","AGE3","AGE4"))
  vt_by_st_ag$age <- factor(vt_by_st_ag$age,levels=c(1:4),labels=c("AGE1","AGE2","AGE3","AGE4"))
  vt_by_st_ag_gd$gender <- factor(vt_by_st_ag_gd$gender,levels=c(0:1),labels=c("MALE","FEMALE"))
  vt_by_st_gd$gender <- factor(vt_by_st_gd$gender,levels=c(0:1),labels=c("MALE","FEMALE"))
  
  return(list(Vote_by_StateAgeGender=vt_by_st_ag_gd,
              Vote_by_StateAge=vt_by_st_ag,
              Vote_by_StateGender=vt_by_st_gd,
              Vote_by_State=vt_by_st))
}
